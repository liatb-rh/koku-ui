import type { Forecast, ForecastData } from '../http/forecast/forecast';
...
function getCostData(val: any, key: string, item?: any) {
  return {
    confidence_max: {
      value: item
...
export function getUnsortedComputedForecastItems<F extends Forecast>({
  ...
  const visitDataPoint = (dataPoint: ForecastData) => {
    ...
    for (const key in dataPoint) {
      if ((dataPoint as any)[key] instanceof Array) {
        return (dataPoint as any)[key].forEach(visitDataPoint);
      }
    }
  };
