...
import { getQueryRoute } from '../http/queries/query';
import type { Location } from 'react-router-dom';
import type { Filter } from './filter';
import { addFilterToQuery, removeFilterFromQuery } from './filter';

export const initQuery = (query: Query, reset: boolean = false, props: any = {}) => {
  const newQuery: any = {
...
export const handleOnCurrencySelect = (query: Query) => initQuery(query);
export const handleOnCostTypeSelect = (query: Query) => initQuery(query);
...
export const handleOnSetPage = (query: Query, report: any, pageNumber: number, isLimit = false) => {
  let limit = 10;
  if (report?.meta) {
...
export const handleOnSort = (query: Query, sortType: string, isSortAscending: boolean, date: string | undefined = undefined) => {
  return initQuery(query, false, {
    order_by: {
